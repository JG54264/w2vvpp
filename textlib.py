...
dogs-x runs'''.split('\n')
    ...
    for t in test_strs:
        print t, '->', TextTool.tokenize(t, clean=True, language='en'), '->', TextTool.tokenize(t, clean=True, language='en', remove_stopword=True)
        
    test_strs = '''一间 干净 整洁 的 房间 。
一只 黄色 的 小狗 趴在 长椅 上'''.split('\n')
    
    for t in test_strs:
        print t, '->', ' '.join(TextTool.tokenize(t, clean=True, language='zh')), '->', ' '.join(TextTool.tokenize(t, clean=True, language='zh', remove_stopword=True))
